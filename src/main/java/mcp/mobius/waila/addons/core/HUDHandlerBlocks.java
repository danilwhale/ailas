...
public class HUDHandlerBlocks implements IComponentProvider {
    ...
    public void appendHead(List<Text> tooltip, IDataAccessor accessor, IPluginConfig config) {
        ...
        ((ITaggableList<Identifier, Text>) tooltip).setTag(OBJECT_NAME_TAG, new LiteralText(String.format(Waila.CONFIG.get().getFormatting().getBlockName(), accessor.getBlock().getName().getFormattedString())));
        if (config.get(PluginCore.CONFIG_SHOW_REGISTRY))
            ((ITaggableList<Identifier, Text>) tooltip).setTag(REGISTRY_NAME_TAG, new LiteralText(Registry.BLOCK.getKey(accessor.getBlock()).toString()).setStyle(new Style().setColor(Formatting.GRAY)));
    }
