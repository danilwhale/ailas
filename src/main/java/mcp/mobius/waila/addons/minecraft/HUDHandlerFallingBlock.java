...
public class HUDHandlerFallingBlock implements IEntityComponentProvider {
    ...
    public ItemStack getDisplayItem(IEntityAccessor accessor, IPluginConfig config) {
        ...
        return new ItemStack(entity.getBlock().getBlock().asItem());
    }

    @Override
    public void appendHead(List<Text> tooltip, IEntityAccessor accessor, IPluginConfig config) {
        FallingBlockEntity entity = (FallingBlockEntity) accessor.getEntity();
        ((ITaggableList<Identifier, Text>) tooltip).setTag(OBJECT_NAME_TAG, new LiteralText(String.format(Waila.CONFIG.get().getFormatting().getEntityName(), entity.getBlock().getBlock().getName().getFormattedString())));
        if (config.get(CONFIG_SHOW_REGISTRY))
            ((ITaggableList<Identifier, Text>) tooltip).setTag(REGISTRY_NAME_TAG, new LiteralText(Registry.ENTITY_TYPE.getKey(accessor.getEntity().getType()).toString()).setStyle(new Style().setColor(Formatting.GRAY)));
    }
