...
public abstract class GuiOptions extends Screen {
    ...
    public void init(Minecraft client, int width, int height) {
        ...
        if (saver != null && canceller != null) {
            addButton(new C_0818095(width / 2 - 100, height - 25, 100, 20, I18n.translate("gui.done"), w -> {
                options.save();
                saver.run();
                onClose();
            }));
            addButton(new C_0818095(width / 2 + 5, height - 25, 100, 20, I18n.translate("gui.cancel"), w -> {
                canceller.run();
                onClose();
            }));
        } else {
            addButton(new C_0818095(width / 2 - 50, height - 25, 100, 20, I18n.translate("gui.done"), w -> {
                options.save();
                onClose();
            }));
        }
    ...
    public void render(int mouseX, int mouseY, float partialTicks) {
        ...
        drawCenteredString(font, title.getFormattedString(), width / 2, 12, 16777215);
        ...
        if (entry instanceof OptionsEntryValue) {
            ...
            if (I18n.hasTranslation(value.getDescription())) {
                ...
                String title = value.getTitle().getFormattedString();
                if (mouseX < valueX || mouseX > valueX + font.getWidth(title))
                    return;

                List<String> tooltip = Lists.newArrayList(title);
                tooltip.addAll(font.split(I18n.translate(value.getDescription()), 200));
