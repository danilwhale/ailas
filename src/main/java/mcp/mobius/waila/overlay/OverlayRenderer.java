...
public class OverlayRenderer {
    ...
    public static void renderOverlay() {
        ...
        if ((mc.screen != null && !(mc.screen instanceof ChatScreen)) || mc.world == null)
            return;

        boolean isOnServer = !mc.isInSingleplayer() || mc.player.networkHandler.getOnlinePlayers().size() > 1;
        if (Waila.CONFIG.get().getGeneral().shouldHideFromPlayerList() && mc.options.playerListKey.isPressed() && isOnServer)
            return;

        if (!Minecraft.isHudDisabled())
            return;
        ...
        if (RayTracing.INSTANCE.getTarget().m_5790247() == HitResult.Type.BLOCK && !RayTracing.INSTANCE.getTargetStack().isEmpty())
            renderOverlay(WailaTickHandler.instance().tooltip);

        if (RayTracing.INSTANCE.getTarget().m_5790247() == HitResult.Type.ENTITY && PluginConfig.INSTANCE.get(PluginCore.CONFIG_SHOW_ENTITY))
            renderOverlay(WailaTickHandler.instance().tooltip);
    }

    public static void renderOverlay(Tooltip tooltip) {
        Minecraft.getInstance().m_2080361().m_8238460("Waila Overlay");
        ...
        Minecraft.getInstance().m_2080361().m_8388219();
    }
